...
const EventInformationScreenCorporate = () => {
  const navigation = useNavigation();

  const [eventName, setEventName] = useState("Missy at 18");
  const [eventDate, setEventDate] = useState('June 10, 2025');
  const [eventTime, setEventTime] = useState('6:00 PM');
  const [eventType, setEventType] = useState('Birthday');
  const [description, setDescription] = useState('Eighteen — the graceful dawn of adulthood, where dreams take flight and purpose begins to bloom.');

  const [venue] = useState('Lilias Function Hall');
  const [venueImage] = useState(require('../assets/eventplaces/lilias.jpg'));
...
export default EventInformationScreenCorporate;
